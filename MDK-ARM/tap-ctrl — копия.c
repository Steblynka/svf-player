...
*/
int MASK_header, SMASK_header, TDI_HDR, TDI_HIR, TDO_header;
int MASK_scan, SMASK_scan, TDI_SDR, TDI_SIR, TDO_scan;
int MASK_TDR, MASK_TIR, SMASK_TDR, SMASK_TIR , TDI_TDR, TDI_TIR, TDO_trailer;
int ENDDRstate = TAP_STATE_RTI;
int ENDIRstate = TAP_STATE_RTI;
 GPIO_InitTypeDef GPIO_InitStruct;
 void inputTDI(int data, int dataSize){
	//ну давайте через ЮАРТ  это закинем
	 	HAL_UART_Transmit_IT(&huart3,  (uint8_t *) data, dataSize);
	 //а вот что с частотой делать?
 }
...
void HDRproc(){
	...
	if (n == 1){
		if (length == 0){
			...
			n = sscanf(wordBuf, "HDR %d %s(%s) %s(%s) %s(%s)",&length,
...
void HIRproc(){
	...
	if (n == 1){
		if (length == 0){
			...
			n = sscanf(wordBuf, "HIR %d %s(%s) %s(%s) %s(%s)",&length,
			temp[0], temp[1], temp[2], temp[3], temp[4],temp[5]);
			//типа прочитали
			for (int i = 0; i < n; i = i + 2){//пока есть прочитанные аргументы
			param[i/2] = strtoul(temp[i+1],NULL, 16); //преобразуем считанное значение из hex в dec
...
void SDRproc(){
	...
	if (n == 1){
		if (length != 0){		//вот тут что-то починить надо	
			n = sscanf(wordBuf, "SDR %d %s(%s) %s(%s) %s(%s)",&length,
			temp[0], temp[1], temp[2], temp[3], temp[4],temp[5]);
			//типа прочитали
			for (int i = 0; i < n; i = i + 2){//пока есть прочитанные аргументы
			param[i/2] = strtoul(temp[i+1],NULL, 16); //преобразуем считанное значение из hex в dec
	...
	//state - > shift DR
	 setTAPtoDRSHIFT();
	HAL_UART_Transmit_IT(&huart3,  (uint8_t *) TDI_HDR, length);
	HAL_UART_Transmit_IT(&huart3,  (uint8_t *) TDI_SDR, length);
	HAL_UART_Transmit_IT(&huart3,  (uint8_t *) TDI_TDR, length);
...
void SIRproc(){
	...
	if (n == 1){
		if (length != 0){			
			n = sscanf(wordBuf, "SIR %d %s(%s) %s(%s) %s(%s)",&length,
			temp[0], temp[1], temp[2], temp[3], temp[4],temp[5]);
			//типа прочитали
			for (int i = 0; i < n; i = i + 2){//пока есть прочитанные аргументы
			param[i/2] = strtoul(temp[i+1],NULL, 16); //преобразуем считанное значение из hex в dec
			...
			if (strncmp(temp[i], "TDI",3) == 0){
				TDI_SIR = param[i/2];
				//передаем это число в IR
				//но сначала переходим в состояние shift ir?
				HAL_UART_Transmit_IT(&huart3,  (uint8_t *) TDI_SIR, length);
			}
			else
				if (strncmp(temp[i], "TDO",3) == 0)
					TDO_scan= param[i/2];
	 ...
	 setTAPtoIRSHIFT();
	HAL_UART_Transmit_IT(&huart3,  (uint8_t *) TDI_HIR, length);
	HAL_UART_Transmit_IT(&huart3,  (uint8_t *) TDI_SIR, length);
	HAL_UART_Transmit_IT(&huart3,  (uint8_t *) TDI_TIR, length);
...
void TDRproc(){
	...
	if (n == 1){
		if (length == 0){
			...
			n = sscanf(wordBuf, "TDR %d %s(%s) %s(%s) %s(%s)",&length,
			temp[0], temp[1], temp[2], temp[3], temp[4],temp[5]);
			//типа прочитали
			for (int i = 0; i < n; i = i + 2){//пока есть прочитанные аргументы
			param[i/2] = strtoul(temp[i+1],NULL, 16); //преобразуем считанное значение из hex в dec
...
void TIRproc(){
	...
	if (n == 1){
		if (length == 0){
			...
			n = sscanf(wordBuf, "TIR %d %s(%s) %s(%s) %s(%s)",&length,
			temp[0], temp[1], temp[2], temp[3], temp[4],temp[5]);
			//типа прочитали
			for (int i = 0; i < n; i = i + 2){//пока есть прочитанные аргументы
			param[i/2] = strtoul(temp[i+1],NULL, 16); //преобразуем считанное значение из hex в dec
